...
(function () {
  ...
  /* ---------------------------------------------------------------------
     PREMIUM CARD 3D TILT: very subtle mouse-follow tilt on bestseller,
     experience, and offer cards. Desktop only, rAF-throttled, disabled
     for touch devices and prefers-reduced-motion.
  --------------------------------------------------------------------- */
  if (canHover && !prefersReducedMotion) {
    const tiltCards = document.querySelectorAll(".bestseller-card, .exp-card, .offer-card");
  ...
  function openItemModal(btn) {
    const d = btn.dataset;
    itemModalIcon.innerHTML = iconSvg(d.itemIcon);
    itemModalCategory.textContent = d.itemCategory || "";
    itemModalName.textContent = d.itemName || "";
    itemModalDesc.textContent = d.itemDesc || "A TYT favorite, made fresh to order.";
  ...
  document.querySelectorAll(".bestseller-link[data-menu-item]").forEach((link) => {
    link.addEventListener("click", (e) => {
      const categoryId = link.dataset.menuCategory;
      const itemName = link.dataset.menuItem;
      if (!itemName) return; // no mapping data — let it behave as a plain #menu link
      e.preventDefault();
      goToMenuItem(categoryId, itemName);
    });
  });
